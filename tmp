...
/* JS Functions, there are 6 ways to declare them:
...
// ****************************************
// Utility functions - using above vars
// ****************************************

function _trace(data)   { if( Config.trace ) { U.debug( U.inspect( data, false, 5 ) ); } }
...
// Base object
// XXX This complains about 'strict violations' as this
// isn't obviously
...
Stream.prototype               = new _LocalListen();
Stream.prototype.constructor   = Stream;

// UDP
function UDP ( type, port, ip) {
    this.connection = Dgram.createSocket("udp4");
    this.port       = port;
    this.ip         = ip;
    this.type       = type;
...
UDP.prototype               = new _LocalListen();
...
(function( config_file, opts ) {
    ...
    Configurator.config( config_file, opts, function( cfg_obj ) {
        ...
        // Full dump of the object, if we're tracing issues.
        _trace( [ "Config object", cfg_obj ] );
        ...
        if( Config.tcp_port ) {
            State.listeners.tcp =
                new Stream( 'tcp', Config.tcp_port, Config.bind_address );
        }
        ...
        if( Config.unix_socket ) {
            State.listeners.socket =
                new Stream( 'socket', Config.unix_socket );
        }
        ...
        if( Config.udp_port ) {
            State.listeners.udp =
                new UDP( 'udp', Config.udp_port, Config.bind_address );
        }
