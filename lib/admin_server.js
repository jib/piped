...
var ___admin_command = exports.___admin_command = function( cmd ) {
    ...
    // keep track of admin connections
        ...
        switch(cmd) {
            ...
            case "stats":
                ...
                for( idx in state.all_servers ) {
                    if( state.all_servers.hasOwnProperty(idx) ) {
                        map[ state.all_servers[idx].name ] =
                            state.all_servers[idx].stats();
                    }
                }
            ...
            default:
                var err = "ADMIN ERROR: UNKNOWN COMMAND " +  cmd + "\n";
                C._debug( err );
                return err;
        }
    } catch(e) {
...
var AdminServer = exports.AdminServer = function( port, address ) {
    ...
    var ll = new LL.Stream( 'admin', port, address, function(ll, conn) {
