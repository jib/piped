...
function _RemoteConnect () {
    ...
    // bind methods
    this.bind = function(methods) {
      methods = methods.split(' ');
      for (var i  = -1, m; m = methods[++i];) {
    ...
    this.mark_available     = function () {
        //if( Config.trace ) { U.debug( "HEALTHY: " + U.inspect( obj ) ); }
    ...
    this.mark_unavailable   = function () {
        //if( Config.trace ) { U.debug( "UNHEALTHY: " + U.inspect( obj ) ); }
...
function LocalUDPListen ( type, port, ip) {
    ...
    this.connection.on( 'message', function (data, rinfo) {
        var remote = _available_server( Config.servers, State.servers );
...
function _LocalListen () {
    this.on_listen  = function (obj) {
        ...
        // it's a local socket
            ...
            U.debug( U.format( "Unix Socket Server started on %s", obj.port ) );
        }
    };
...
(function( config_file, opts ) {
    ...
    Configurator.config( config_file, opts, function( cfg_obj ) {
        ...
        (function() {
            ...
            if( Config.trace ) {
                U.debug( U.inspect( Config.servers ) );
            }
