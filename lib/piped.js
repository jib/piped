...
function LocalStreamListen (type, port, ip) {
    ...
    this.connection.on( 'connection', function( conn ) {
        ...
        for( idx in remotes ) {
            ...
            // send it to the overflow log if we haven't yet for this message
            } else {
                if( !fail++ && Config.overflow_stream ) {
                    if( Config.trace ) { U.debug( "Piping to overflow log" ); }
                    Config.overflow_stream.incr_stats();
                    conn.pipe( Config.overflow_stream.connection, { end: false } );
                }
            }
        }
    }.bind(this));
    ...
    this.connection.on( 'message', function (data, rinfo) {
        ...
        for( idx in remotes ) {
            ...
            // send it to the overflow log if we haven't yet for this message
                ...
                if( !fail++ && Config.overflow_stream ) {
                    if( Config.trace ) { U.debug( "Sending to overflow log" ); }
                    Config.overflow_stream.incr_stats();
                    Config.overflow_stream.send( data );
                }
            }
        }
    }.bind(this));
...
function _available_server ( cfg_servers, servers) {
    ...
    for( idx in cfg_servers ) {
        ...
        if( rv[ sub_idx ] === undefined ) {
            // if we got here, we couldn't send the message
            U.error( U.format( "No available servers in set " + aref ) );
            Stats.connections.failed++;

            // we push 'undefined', so the fanout functions keep working
            // correctly, and fallback to the overflow log. Reason we're
            // not returning the overflow stream here, is that it might
            // be returned in multiple positions in rv (ie, if multiple
            // endpoint chains fail), resulting in multiple logging of
            // the same data in the overflow log
            rv[ sub_idx ] = undefined;
        }
    }
