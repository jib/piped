...
var RemotePool = exports.RemotePool =
    ...
    obj.health_check = function() {
        ...
        for( i = 0; i < this.servers.length; i++ ) {
            ...
            for( j = 0; j < ary.length; j++ ) {
                ...
                // First, check if the remote is slow. If so, we just have
                // to wait until you're buffer is drained. The check will
                // mark it as not slow if the buffer is drained (enough).
                if( remote.check_if_slow() === true ) {
                    continue;
                }

                // Now, if we already have a healthy remote in this chain,
                // we don't need to do the other checks right now.
    ...
    obj.reconnect_to_servers = function() {
        ...
        // XXX jslint: The body of a for in should be wrapped in an if statement
        // to filter unwanted properties from the prototype. - find & fix
        var name;
        for( name in state.all_servers ) {
            var remote = state.all_servers[name];

            // is it backed up? Slowness marker is removed during health check
            if( remote.is_slow ) {
                C._trace( U.format( "Remote %s is slow (bufsize=%s)",
                    name, remote.connection.bufferSize
                ) );
                slow++

            // otherwise, it might be probably down
            } else if ( remote.is_down ) {
                C._trace( "Remote is down: " + name );
    ...
    obj.send_health_stats = function() {
        ...
        if( statsd ) {
            ...
            statsd.send( U.format('%spool.remotes.healthy%s:%s|g',
                prefix, suffix, state.health_check.healthy_remotes || 0) );
            statsd.send( U.format('%spool.remotes.unhealthy%s:%s|g',
                prefix, suffix, state.health_check.unhealthy_remotes || 0) );

            // Number of slow & unavailable nodes sent as stats -
            // set by obj.reconnect_to_servers
            statsd.send( U.format('%spool.remotes.slow%s:%s|g',
                prefix, suffix, state.health_check.slow_remotes || 0 ) );
            statsd.send( U.format('%spool.remotes.unavailable%s:%s|g',
                prefix, suffix, state.health_check.unavailable_remotes || 0 ) );

            // Number of healthy & unhealthy chains - obj.health_check
            statsd.send( U.format('%spool.chains.healthy%s:%s|g',
                prefix, suffix, state.health_check.healthy_chains || 0 ) );
            statsd.send( U.format('%spool.chains.unhealthy%s:%s|g',
                prefix, suffix, state.health_check.unhealthy_chains || 0 ) );
        }
    }.bind(obj);
