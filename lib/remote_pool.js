...
function _connect_to_server( str, is_reconnect ) {
    ...
    if( config.trace ) {
        ...
        C._log( U.format( "Connecting to remote %s", str ) );
    }
    ...
    var remote = (function( str ) {
        ...
        if( m && m[0] ) {
            ...
            switch( m[1] ) {
                case 'socket':
                    return new RS.RemoteStreamSend( str, conn, conn, is_reconnect );
                case 'tcp':
                    return new RS.RemoteStreamSend( str, port, host, is_reconnect );
                case 'udp':
                    return new RS.RemoteUDPSend( str, port, host, is_reconnect );
                case 'file':
    ...
    obj.connect_to_servers = function (func) {
        ...
        for( i = 0; i < this.servers.length; i++ ) {
            ...
            for( j = 0; j < ary.length; j++ ) {
                var remote = _connect_to_server.bind(this)( ary[j] );
